...
package ${lname}_reg_pkg;
  ...
  % if len(r.children) == 1:
    % if r.children[0].inner.implements_storage:
    logic ${sv_bitarray(r.children[0])}q;
    % endif
    % if r.children[0].needs_qe:
    logic qe;
    % endif
    % if r.children[0].needs_qre:
    logic re;
    % endif
  % else:
    ## Only create the substruct if the field is present in reg2hw struct
    % for f in filter(lambda f : f.present_in_reg2hw, reversed(r.children)):
    struct packed {
      % if f.inner.implements_storage:
  ...
  typedef struct packed {
    % if len(r.children) == 1:
    logic ${sv_bitarray(r.children[0])} d;
    % if r.children[0].inner.implements_storage:
    logic de;
    % endif
    % else:
    % for f in reversed(r.children):
    struct packed {
...
<%
    if r.is_wide:
      ...
      for i in range(r.subregs):
        values.append(f"{addr_width}'h{r.inner.absolute_address+(i*r.addressincr):X}")
        params.append(f"{ublock}_{r.path().upper()}_{i}_OFFSET")
    else:
      values = [f"{addr_width}'h{r.inner.absolute_address:X}"]
...
${reg.inner.owning_addrmap.inst_name.lower()}_hw2reg_${reg.path().lower()}_t
</%def>\
<%def name="reg2hw_t_gen(reg)" filter="trim">\
${reg.inner.owning_addrmap.inst_name.lower()}_reg2hw_${reg.path().lower()}_t
</%def>\
